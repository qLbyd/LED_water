...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
  ...
  if(GPIO_Pin == SWITCH_Pin){
     ...
     GPIO_PinState pinState = HAL_GPIO_ReadPin(SWITCH_GPIO_Port,SWITCH_Pin);
      ...
      //高电位
      if(pinState==GPIO_PIN_SET)//PB14高电位流水灯亮
      {
		  flag=1;//PB14高电位标识
	  }
      //低电位
      else if(pinState==GPIO_PIN_RESET)//PB14低电位流水灯灭
	  {
		  flag=0;//PB14低电位标识
	  }
      } 
...
if(flag==1)
	  ...
	  {
          HAL_GPIO_WritePin(GPIOA,GPIO_PIN_0,GPIO_PIN_RESET);//PA0熄灯
          HAL_GPIO_WritePin(GPIOB,GPIO_PIN_6,GPIO_PIN_RESET);//PA1熄灯
					HAL_GPIO_WritePin(GPIOC,GPIO_PIN_14,GPIO_PIN_RESET);//PA2熄灯
	  }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
